...
make_sample_lr_prod_activity_plots <- function(prioritization_tables, prioritized_tbl_oi, widths = NULL) {
  ...
  group_data_frac_expression <- prioritization_tables$group_prioritization_table_source %>%
    dplyr::mutate(sender_receiver = paste(sender, receiver, sep = " --> "), lr_interaction = paste(ligand, receptor, sep = " - ")) %>%
    dplyr::distinct(id, sender, receiver, sender_receiver, lr_interaction, group, fraction_ligand_group, fraction_receptor_group) %>%
    dplyr::filter(id %in% sample_data$id) %>%
    dplyr::arrange(receiver) %>%
    dplyr::group_by(receiver) %>%
    dplyr::arrange(sender, .by_group = TRUE)
  ...
  cs_data <- group_data %>%
    distinct(sender_receiver, lr_interaction, group, scaled_pb_ligand, scaled_pb_receptor) %>%
  ...
  frac_data <- group_data %>%
    distinct(sender_receiver, lr_interaction, group, fraction_ligand_group, fraction_receptor_group) %>%
  ...
  p_cs <- cs_data %>%
    ...
    labs(size = "Fraction of expression")
...
make_sample_lr_prod_activity_plots_Omnipath <- function(prioritization_tables, prioritized_tbl_oi, widths = NULL) {
  ...
  group_data_omnipath <- prioritization_tables$group_prioritization_table_source %>%
    dplyr::mutate(sender_receiver = paste(sender, receiver, sep = " --> "), lr_interaction = paste(ligand, receptor, sep = " - ")) %>%
    dplyr::distinct(id, sender, receiver, sender_receiver, lr_interaction) %>%
    dplyr::filter(id %in% sample_data$id) %>%
    inner_join(omnipath_df) %>%
    dplyr::arrange(receiver) %>%
    dplyr::group_by(receiver) %>%
    dplyr::arrange(sender, .by_group = TRUE)
  ...
  omnipath_data <- group_data %>%
    distinct(sender_receiver, lr_interaction, curation_effort, n_references, n_resources) %>%
    tidyr::gather(omnipath_score_type, omnipath_score, curation_effort:n_resources)
...
make_sample_lr_prod_activity_batch_plots <- function(prioritization_tables, prioritized_tbl_oi, grouping_tbl, batch_oi, widths = NULL, heights = NULL) {
  ...
  cs_data <- group_data %>%
    ...
    distinct(sender_receiver, lr_interaction, group, scaled_pb_ligand, scaled_pb_receptor) %>%
    gather(LR, celltype_specificity, scaled_pb_ligand:scaled_pb_receptor)
  cs_data$LR[cs_data$LR == "scaled_pb_ligand"] <- "ligand"
  cs_data$LR[cs_data$LR == "scaled_pb_receptor"] <- "receptor"
...
make_ligand_activity_plots <- function(prioritization_tables, ligands_oi, contrast_tbl, widths = NULL) {
  requireNamespace("dplyr")
  requireNamespace("ggplot2")
  group_data <- prioritization_tables$ligand_activities_target_de_tbl %>%
    dplyr::inner_join(contrast_tbl) %>%
    dplyr::distinct(group, ligand, receiver, activity, activity_scaled, direction_regulation) %>%
    dplyr::filter(ligand %in% ligands_oi) %>%
    dplyr::mutate(group_direction_regulation = paste(group, direction_regulation, sep = "-"))
...
make_DEgene_dotplot_pseudobulk_reversed <- function(genes_oi, celltype_info, prioritization_tables, celltype_oi, grouping_tbl, groups_oi = NULL, target_regulation_df = NULL) {
  ...
  if (is.null(target_regulation_df)) {
    ...
    plot_data <- plot_data %>%
      dplyr::filter(gene %in% genes_oi & celltype %in% celltype_oi) %>%
...
make_ligand_receptor_violin_plot <- function(sce, ligand_oi, receptor_oi, sender_oi, receiver_oi, group_oi, group_id, sample_id, celltype_id, batch_oi = NA, background_groups = NULL) {
  ...
  if (is.null(background_groups)) {
    background_groups <- SummarizedExperiment::colData(sce)[, group_id] %>%
      unique() %>%
      generics::setdiff(group_oi)
  }
  ...
  if (is.na(batch_oi)) {
    p_sender <- exprs_df %>% ggplot(aes(sample_id, expression, group = sample_id, color = group_id)) +
    ...
    extra_metadata <- SummarizedExperiment::colData(sce_subset) %>%
      tibble::as_tibble() %>%
      dplyr::select(all_of(sample_id), all_of(batch_oi)) %>%
      dplyr::distinct() %>%
  ...
  if (is.na(batch_oi)) {
    p_receiver <- exprs_df %>% ggplot(aes(sample_id, expression, group = sample_id, color = group_id)) +
      geom_violin(color = "grey10") +
      ggbeeswarm::geom_quasirandom(bandwidth = 1.25, varwidth = TRUE) +
      facet_grid(. ~ group_id, scales = "free", space = "free") +
      scale_x_discrete(position = "bottom") +
...
make_target_violin_plot <- function(sce, target_oi, receiver_oi, group_oi, group_id, sample_id, celltype_id, batch_oi = NA, background_groups = NULL) {
  ...
  if (is.na(batch_oi)) {
    p_violin <- exprs_df %>% ggplot(aes(sample_id, expression, group = sample_id, color = group_id)) +
      geom_violin(color = "grey10") +
      ggbeeswarm::geom_quasirandom(bandwidth = 1.25, varwidth = TRUE) +
      facet_grid(. ~ group_id, scales = "free", space = "free") +
      scale_x_discrete(position = "bottom") +
...
make_lr_target_scatter_plot <- function(prioritization_tables, ligand_oi, receptor_oi, sender_oi, receiver_oi, receiver_info, grouping_tbl, lr_target_prior_cor_filtered) {
  ...
  ligand_receptor_pb_prod_df <- prioritization_tables$sample_prioritization_tbl %>%
    dplyr::filter(keep_receiver == 1 & keep_sender == 1) %>%
    dplyr::filter(ligand == ligand_oi, receptor == receptor_oi, sender == sender_oi, receiver == receiver_oi) %>%
    dplyr::distinct(sample, ligand_receptor_pb_prod, id)

  targets_oi <- ligand_receptor_pb_prod_df %>%
    dplyr::inner_join(lr_target_prior_cor_filtered, by = "id") %>%
    dplyr::pull(target) %>%
    unique()

  target_pb_df <- receiver_info$pb_df %>%
    dplyr::inner_join(grouping_tbl, by = "sample") %>%
    dplyr::filter(keep_receiver == 1 & keep_sender == 1) %>%
    dplyr::filter(gene %in% targets_oi & celltype == receiver_oi) %>%
    dplyr::distinct(gene, sample, pb_sample) %>%
    dplyr::rename(target_pb = pb_sample)

  ligand_receptor_target_pb_df <- grouping_tbl %>%
    dplyr::inner_join(ligand_receptor_pb_prod_df, by = "sample") %>%
    dplyr::inner_join(target_pb_df, by = "sample")
...
make_lr_target_prior_cor_heatmap <- function(lr_target_prior_cor_filtered, add_grid = TRUE) {
  ...
  if (add_grid == TRUE) {
    p_lr_target <- lr_target_prior_cor_filtered %>%
      dplyr::mutate(sender_receiver = paste(sender, receiver, sep = " --> "), lr_interaction = paste(ligand, receptor, sep = " - ")) %>%
...
make_lr_target_correlation_plot <- function(prioritization_tables, prioritized_tbl_oi, lr_target_prior_cor_filtered, grouping_tbl, receiver_info, receiver_oi, plot_legend = TRUE, heights = NULL, widths = NULL) {
  ...
  sample_data <- sample_data %>%
    ...
    dplyr::mutate(sample = factor(sample, levels = rev(levels(sample)))) # reverse - to have the same order as used for the targets

  ##### LR-->Target plot

  lr_target_data <- lr_target_prior_cor_filtered %>%
    dplyr::inner_join(sample_data, by = c("sender", "receiver", "ligand", "receptor", "id", "group")) %>%
    dplyr::select(id, lr_interaction, sender, receiver, sender_receiver, group, target, prior_score, pearson, direction_regulation) %>%
    dplyr::distinct() %>%
  ...
  if (plot_legend == FALSE) {
    ...
    combined_plot <- patchwork::wrap_plots(
      A = p_lr_exprs + theme(legend.position = "none", axis.ticks = element_blank()) + theme(axis.title.x = element_text()),
...
make_ggraph_ligand_target_links <- function(lr_target_prior_cor_filtered, prioritized_tbl_oi, colors) {
  ...
  source_df_lr <- prioritized_tbl_oi %>%
    dplyr::mutate(celltype_ligand = paste(sender, ligand, sep = "_"), celltype_receptor = paste(receiver, receptor, sep = "_")) %>%
    dplyr::select(group, sender, receiver, celltype_ligand, celltype_receptor, ligand, receptor)
  source_df_lrt <- lr_target_prior_cor_filtered %>%
    dplyr::mutate(celltype_ligand = paste(sender, ligand, sep = "_"), celltype_target = paste(receiver, target, sep = "_"), celltype_receptor = paste(receiver, receptor, sep = "_")) %>%
  ...
  ligand_receptor_network <- lr_gr_network %>%
    dplyr::filter(celltype_receptor %in% celltype_target) %>%
    dplyr::select(celltype_ligand, celltype_receptor, direction_regulation, group) %>%
    dplyr::distinct() %>%
    dplyr::rename(sender = celltype_ligand, receiver = celltype_receptor) %>%
    dplyr::mutate(type = "Ligand-Receptor", weight = 1)

  ligand_target_network <- lr_gr_network %>%
    dplyr::select(celltype_ligand, celltype_target, direction_regulation, group) %>%
    dplyr::distinct() %>%
    dplyr::rename(sender = celltype_ligand, receiver = celltype_target) %>%
    dplyr::mutate(type = "Ligand-Target", weight = 1)

  links <- ligand_target_network %>% dplyr::bind_rows(ligand_receptor_network)
  nodes <- lr_gr_network %>%
    dplyr::select(celltype_ligand, sender, ligand) %>%
    ...
    dplyr::distinct() %>%
    dplyr::filter(node %in% c(links$sender, links$receiver))
  ...
  nodes <- nodes %>%
    data.frame() %>%
    magrittr::set_rownames(nodes$node)
...
#' ggraph_signaling_path <- make_ggraph_signaling_path(active_signaling_network_min_max, colors)
#' }
#'
#' @export
#'
make_ggraph_signaling_path <- function(signaling_graph_list, colors, ligands_all, receptors_all, targets_all) {
  ...
  edge_df <- dplyr::bind_rows(
    signaling_graph_list$sig %>% dplyr::mutate(interaction_type = "signaling"),
...
make_ligand_activity_target_plot <- function(group_oi, receiver_oi, prioritized_tbl_oi, prioritization_tables, ligand_activities_targets_DEgenes, contrast_tbl, grouping_tbl, receiver_info, ligand_target_matrix, groups_oi = NULL, plot_legend = TRUE, heights = NULL, widths = NULL) {
  ...
  vis_ligand_target_df <- vis_ligand_target %>%
    data.frame() %>%
  ...
  ligand_activity_df <- ligand_activities_targets_DEgenes$ligand_activities %>%
    dplyr::ungroup() %>%
    dplyr::filter(ligand %in% order_ligands_ & receiver == receiver_oi) %>%
    dplyr::inner_join(contrast_tbl) %>%
    dplyr::filter(group %in% groups_oi) %>%
  ...
  # Combine the plots -----
  n_groups <- ligand_activity_df$group %>%
    unique() %>%
    length()
  ...
  if (plot_legend == FALSE) {
    design <- "AaB
               ##C"
    combined_plot <- patchwork::wrap_plots(
      A = p_ligand_activity_scaled + theme(legend.position = "none", axis.ticks = element_blank()) + theme(axis.title.x = element_text()),
...
make_circos_group_comparison <- function(prioritized_tbl_oi, colors_sender, colors_receiver) {
  ...
  legend <- ComplexHeatmap::Legend(
    ...
    type = "grid",
    legend_gp = grid::gpar(fill = colors_receiver[prioritized_tbl_oi$receiver %>%
      unique() %>%
      sort()]),
  ...
  legend <- ComplexHeatmap::Legend(
    ...
    type = "grid",
    legend_gp = grid::gpar(fill = colors_sender[prioritized_tbl_oi$sender %>%
      unique() %>%
      sort()]),
    title_position = "topleft",
    title = "Sender"
...
#'
#' @return a list <U+00F2>f plots for each celltype-contrast pair: an upset plot and line plot are drawn.
#'
#' @import ggplot2
#' @import dplyr
#' @importFrom tibble tibble
...
compare_normal_emp_pvals <- function(DE_info, DE_info_emp, adj_pval = FALSE) {
  ...
  comparison_plots <- DE_info$celltype_de$de_output_tidy$cluster_id %>%
    unique() %>%
    lapply(function(celltype_oi, adjusted = FALSE) {
      if (adjusted == TRUE) {
        de_genes_normal <- DE_info$celltype_de$de_output_tidy %>%
          dplyr::filter(cluster_id == celltype_oi) %>%
          dplyr::filter(p_adj <= 0.05) %>%
          dplyr::pull(gene) %>%
          unique()
        de_genes_emp <- DE_info_emp$de_output_tidy_emp %>%
          dplyr::filter(cluster_id == celltype_oi) %>%
          dplyr::filter(p_adj_emp <= 0.05) %>%
          dplyr::pull(gene) %>%
          unique()
      } else {
        de_genes_normal <- DE_info$celltype_de$de_output_tidy %>%
      ...
      upset_df <- tibble::tibble(gene = union(de_genes_normal, de_genes_emp), normal = as.double(gene %in% de_genes_normal), empirical = as.double(gene %in% de_genes_emp)) %>%
        data.frame() %>%
      ...
      p_ranking <- DE_info_emp$de_output_tidy_emp %>%
        dplyr::filter(gene %in% union(de_genes_normal, de_genes_emp) & cluster_id == celltype_oi) %>%
        ...
        dplyr::select(cluster_id, contrast, gene, rank_type, rank, empirical_lower) %>%
